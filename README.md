...
claude_gateway = {
  ...
  # Generate both with: openssl rand -hex 16
  # gateway_api_key    — static bearer token for direct (non-OIDC) API calls.
  # gateway_jwt_secret — HMAC-HS256 key used to sign session JWTs (must be 32 chars).
  gateway_api_key    = "REPLACE_WITH_STRONG_KEY"
  gateway_jwt_secret = "REPLACE_WITH_32_CHAR_SECRET"
